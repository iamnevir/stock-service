...
def worker_compute_process(index, config, gen, alpha_name, fee, dic_freqs, df_tick, stop_loss, DIC_ALPHAS, return_dict):
    ...
    if gen == "1_3":
        freq, inertia, *rest = parts
        freq, inertia = int(freq), float(inertia)
...
class Simulator:
    def __init__(self, alpha_name,configs ,fee=0.1, dic_freqs=None,DIC_ALPHAS=None,df_tick=None,start=None,end=None,stop_loss=0,gen='gen1_2',booksize=None):
    ...
    def compute_mega(self):
        ...
        df_alpha['position'] = ((df_pos_sum['position'] / len(self.configs)  * self.booksize).round(6).astype(int))
...
class Alpha_Domains:
    ...
    @staticmethod
    def compute_performance(df_alpha, start=None, end=None,equity =300,df_1d=None,stop_loss=0):
        lst_errs = []
