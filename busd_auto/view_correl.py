...
def view_wfa_correlation(id,start,end):
    ...
    local_db = MongoClient(get_mongo_uri())['busd']
    ...
    docs = list(coll.find({"_id": {"$in": selected_ids}}, {"_id": 1, "strategy": 1, "sharpe": 1, "tvr": 1}))
    id_to_label = {doc["_id"]: (doc["strategy"] if doc.get("strategy") else str(doc["_id"])) for doc in docs}
    ...
    unique_configs = list({doc.get("strategy") for doc in triangle_docs if doc.get("strategy") is not None})

    sharpe_values = [
        doc.get("sharpe", 0.0)
        for doc in triangle_docs if doc.get("sharpe") is not None
    ]
    # print(sharpe_values)
    ...
    tvr_values = [
        doc.get("tvr", 0.0)
        for doc in triangle_docs if doc.get("tvr") is not None
    ]
    ...
    busd_collection.update_one(
            ...
            {"$set": {
                ...
                "wfa.$.book_size": len(results.get("strategies", [])),
            }}
        )    
    return True