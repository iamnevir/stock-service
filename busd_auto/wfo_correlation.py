...
from busd_auto.mega import os_wfo_backtest
from busd_auto.utils import get_mongo_uri, load_data, make_key, setup_logger, send_telegram_message
from busd_auto.view_correl import view_wfo_correlation
...
def correlation(id):
    ...
    coll = busd_db["correlation_backtest"]
    ...
    send_telegram_message(f"✅ Correlation hoàn tất cho: {name} trong {time.time() - start_time:.2f}s.")
    mongo_client.close()
    view_wfo_correlation(id=id)
    os_wfo_backtest(id=id)
    
def main():
    if len(sys.argv) < 2:
        print("Usage: /home/ubuntu/anaconda3/bin/python /home/ubuntu/nevir/busd_auto/wfo_correlation.py <_id>")
        sys.exit(1)
